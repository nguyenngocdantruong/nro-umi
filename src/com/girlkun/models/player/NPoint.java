...
public class NPoint {
    ...
    private void setPointWhenWearClothes() {
        ...
        // Buff Bánh Trung Thu
        if (this.player.effectSkill != null && this.player.effectSkill.isTrungThu) {
            switch (this.player.effectSkill.trungThuType) {
                case 1: // Bánh 1 trứng: +10% sức đánh
                    this.tlDame.add(10);
                    break;
                case 2: // Bánh 2 trứng: +15% sức đánh
                    this.tlDame.add(15);
                    break;
                case 3: // Bánh Gà quay: +15% KI
                    this.tlMp.add(15);
                    break;
                case 4: // Bánh Thập cẩm: +15% HP, KI, sức đánh
                    this.tlHp.add(15);
                    this.tlMp.add(15);
                    this.tlDame.add(15);
                    break;
                case 5: // Bánh Đặc biệt: +20% HP, +15% sức đánh
                    this.tlHp.add(20);
                    this.tlDame.add(15);
                    break;
            }
    ...
    private void setHpMax() {
        ...
        if(this.player.itemTime != null){
            ...
            // Bánh trung thu
            if (this.player.itemTime.isUseDacbiet) {
                this.hpMax += (this.hpMax / 5);
            }
            if (this.player.itemTime.itemTimeEvent.isUseBanhTrungThuThapCam) {
    ...
    private void setMpMax() {
        ...
        // Bánh trung thu
        if(this.player.itemTime != null){
            int z = (this.player.itemTime.itemTimeEvent.isUseBanhTrungThuGaQuay ? 1 : 0) + (this.player.itemTime.itemTimeEvent.isUseBanhTrungThuThapCam ? 1 : 0);
            this.mpMax += z * (this.mpMax * 15 / 100);
        }
    }
    ...
    private void setDame() {
        ...
        if(this.player.itemTime != null && this.player.itemTime.itemTimeEvent.isUseBanhTrungThuThapCam){
            this.dame += ((long) this.dame * 15 / 100);
        }
        // cuồng nộ
        ...
        // Bị biến thành củ cà rốt giảm 15% dame
        if (this.player.effectSkin.isCarrot) {
            this.dame -= ((long) this.dame * 15 / 100);
        }
        ...
        if (this.player.zone != null && MapService.gI().isMapDiaNguc(this.player.zone.map.mapId) && !this.isKhongNong) {
            this.dame /= 2.5;
        }
        // ngọc rồng đen 1 sao
        if (this.player.rewardBlackBall.timeOutOfDateReward[0] > System.currentTimeMillis()) {
    ...
    private void resetPoint() {
        ...
        // custom-drsylas
        this.tlSDDep.clear();
        if (this.player.effectSkin.hasBulmaNear())
            this.tlSDDep.add(15);
        this.hasBuiBui = false;
        this.IsBiLamCham = false;
        this.tlBanTrung = (short) (this.player.effectSkin.hasXinbatoNear() ? 30 : 100);
    }
    ...
    public int getDameAttack(boolean isAttackMob) {
        ...
        if (player.isPl() && dameAttack >= 50000000) {
            ServerNotify.gI().notify(player.name + " đã đánh một chiêu " + player.playerSkill.skillSelect.template.name
                    + " với sát thương " + Util.numberToMoney(dameAttack) + " mọi người đều ngưỡng mộ");
        }
    ...
    public long calSucManhTiemNang(long tiemNang) {
        if (power < getPowerLimit()) {
            ...
            tiemNang *= EventManager.gI().getCurrentEvent().getHeSoTnSm();
